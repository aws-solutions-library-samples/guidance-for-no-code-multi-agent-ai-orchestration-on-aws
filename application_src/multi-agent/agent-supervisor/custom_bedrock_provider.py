...
from config import ANTHROPIC_MODELS, MODEL_COOLDOWN_SECONDS
...
class CustomBedrockModel(Model):
    ...
    def _initialize_client(self):
        """Initialize boto3 Bedrock client with zero retries for immediate throttling detection."""
        try:
            self.client = boto3.client(
                ...
                config=BotocoreConfig(
                    retries={
                        'max_attempts': 0,  # Zero retries - fail immediately on throttling
                        'mode': 'standard'
                    },
                    read_timeout=120,   # Allow legitimate long responses
                    connect_timeout=10, # Reasonable connection timeout
                    max_pool_connections=50
                )
            )
            logger.info(f"✅ Custom Bedrock client initialized for {self.config['model_id']}")
        except Exception:
            logger.exception("❌ Failed to initialize custom Bedrock client")
            raise
        ...
        **kwargs: Any
        ...
        try:
            ...
            if self._is_throttling_error(exc):
                ...
                raise ModelThrottledException(f"Custom Bedrock throttling detected") from exc
            else:
...
class ModelSwitchingBedrockProvider:
    ...
    def __init__(self, available_models: Optional[list[str]] = None, model_manager=None):
        ...
        Args:
            available_models: List of available Bedrock model IDs (defaults to ANTHROPIC_MODELS from config)
        ...
        logger.info(f"✅ Model Switching Bedrock Provider initialized with {len(self.available_models)} models from config")
    ...
    def put_model_in_cooldown(self, model_id: str, cooldown_seconds: Optional[int] = None):
        ...
        logger.warning(f"🚨 Model {model_id} in cooldown for {cooldown_seconds}s (using config)")
...
class SwitchingBedrockModel(Model):
    ...
    def _switch_to_model(self, model_id: str):
        ...
        try:
            ...
            logger.exception(f"❌ Failed to switch to model {model_id}")
            raise
